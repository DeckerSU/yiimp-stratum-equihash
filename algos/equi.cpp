...
void equi_hash(const char* input, char* output, uint32_t len)
{
       ...
       sha256_hash(input, (char *)output1, len);
       sha256_hash((const char *)output1, output, 32);
...
// soln -> equihash solution (excluding 3 bytes with size, so 1344 bytes length)
  ...
  expandArray((const unsigned char *)soln, equihashSolutionSize, (unsigned char *)&indices, sizeof(indices), collisionBitLength + 1, 1);
